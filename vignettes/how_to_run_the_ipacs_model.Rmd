...
``` {r run_visit}
visit_start_time <- Sys.time()
visit_sim_result <- run_visit_sim()
visit_end_time <- Sys.time()
time_taken <- visit_end_time - visit_start_time
print(time_taken)
...
Use `save_visit()` to extract results for each date for number in queue, occupancy, wait and costs and then save to csv.
``` {r output_visit}
